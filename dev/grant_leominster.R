## Grant figure: Leominster (commuter rail with transit station)
##
## Leominster is a commuter rail community with gross_slack=1.27 but
## net_slack=0.91 — technically compliant on zoned capacity but 1,094
## existing housing units inflate the numbers. 34% of capacity is in
## the station area. Illustrates "paper compliance" with station constraint.
...
make_district_map <- function(parcels_sf, muni_boundary, station_clipped,
                              in_locs, subtitle = "") {
  district_sf <- sf::st_union(parcels_sf[parcels_sf$LOC_ID %in% in_locs, ])
...
pct_label <- paste0(
  "Adopted plan: ",
  scales::ordinal(round(adopted_percentile * 100)),
  " percentile"
)
...
ggsave(
  "dev/grant_leominster_figure.png",
  plot = final_figure,
  width = 11, height = 4.5, units = "in", dpi = 300
)
ggsave(
  "dev/grant_leominster_figure.pdf",
  plot = final_figure,
