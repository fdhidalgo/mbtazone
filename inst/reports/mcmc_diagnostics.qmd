...
# Dynamic chain colors: accommodate both k>0 and k=0 chains
...
if (length(k0_names) > 0 && length(kpos_names) > 0) {
  ...
  # Build comparison trajectory plot
  traj_k0_list <- lapply(seq_along(k0_names), function(i) {
    ch <- parcel_chain_results[[k0_names[i]]]
    n_sec <- ch$diagnostics$n_secondaries_trajectory
    n_steps <- length(n_sec)
    idx <- seq(1, n_steps, by = max(1, n_steps %/% 500))
    data.table::data.table(
      step = idx,
      n_secondaries = n_sec[idx],
      chain = k0_names[i],
      type = "k=0"
    )
  })

  traj_kpos_list <- lapply(seq_along(kpos_names), function(i) {
    ch <- parcel_chain_results[[kpos_names[i]]]
    n_sec <- ch$diagnostics$n_secondaries_trajectory
    n_steps <- length(n_sec)
    idx <- seq(1, n_steps, by = max(1, n_steps %/% 500))
    data.table::data.table(
  ...
  traj_compare <- data.table::rbindlist(c(traj_kpos_list, traj_k0_list))

  # Use named chain colors palette
  all_chain_names <- c(kpos_names, k0_names)
  kpos_cols <- c("#e41a1c", "#377eb8", "#4daf4a", "#984ea3")[seq_along(kpos_names)]
  k0_cols   <- c("#d95f02", "#8c510a")[seq_along(k0_names)]
  names(kpos_cols) <- kpos_names
  names(k0_cols)   <- k0_names
  compare_colors <- c(kpos_cols, k0_cols)

  p <- ggplot(traj_compare, aes(x = step, y = n_secondaries, color = chain)) +
    geom_line(alpha = 0.7, linewidth = 0.4) +
    scale_color_manual(values = compare_colors, name = "Chain") +
    labs(
      title = "Secondary Count: k>0 vs k=0 Initialized Chains",
      x = "MCMC Step",
      y = "Number of Secondaries"
    ) +
    theme_minimal()
  ...
  if (k0_mean_k < 3) {
    cat("\n\n**k=0 chains stayed at low k** (mean k =", round(k0_mean_k, 2),
        "after burn-in). The posterior is likely multimodal in k; the k=0 / low-k ",
        "region is a valid mode that k>0 chains cannot reach.\n\n")
  } else {
    cat("\n\n**k=0 chains drifted up** to mean k =", round(k0_mean_k, 2),
        " (comparable to k>0 chains at", round(kpos_mean_k, 2),
        "). k=0 has low posterior mass; high-k dominates.\n\n")
  }
} else {
  cat("\n\n*No k=0 chains detected — section skipped.*\n\n")
}
```
...
# Chain colors (k>0 chains + k=0 chains)
chain_colors <- c(
  chain_1 = "#e41a1c",
  chain_2 = "#377eb8",
  chain_3 = "#4daf4a",
