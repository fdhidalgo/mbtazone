...
source(
  find_repo_file("inst/targets/temp_targets_config.R"),
  local = config_env
)
source(
  find_repo_file("inst/targets/temp_targets_parcel_config.R"),
  local = config_env
)
...
district_paths <- tar_read(district_paths)
district_data <- tar_read(district_data)
adjacency_graph <- tar_read(adjacency_graph)
...
# Helper: extract a trajectory field from all valid chains,
# discarding the first MCMC_BURN_IN draws from each chain.
...
This report presents diagnostic results for MCMC sampling of feasible zoning plans. The sampler explores parcel configurations satisfying statutory constraints.
...
| Secondary-count prior λ (k) | `r K_PRIOR_LAMBDA` |
| Secondary minimum area | `r SECONDARY_AREA_THRESHOLD` acres |
| Discovery capacity multiplier | `r DISCOVERY_CAPACITY_MULTIPLIER`× |
| MCMC steps per chain | `r scales::comma(MCMC_STEPS_MACRO)` |
| Burn-in | `r scales::comma(MCMC_BURN_IN)` |
...
# The new pipeline stores parcels directly in the gpkg (PublicInst, UseDesc are present)
raw_parcels <- sf::st_read(district_paths$gpkg, layer = "parcels", quiet = TRUE)
...
district_parcels$is_institutional[is.na(
  district_parcels$is_institutional
)] <- FALSE
...
TARGET_DENSITY <- 15
district_parcels$raw_density <- district_parcels$capacity / district_parcels$area
district_parcels$raw_density[
  is.na(district_parcels$raw_density) |
    is.infinite(district_parcels$raw_density)
] <- 0
district_parcels$density_contribution <- district_parcels$raw_density -
  TARGET_DENSITY

# Pre-compute rounded values for tooltips
district_parcels$raw_density_rounded <- round(district_parcels$raw_density, 1)
district_parcels$density_contribution_rounded <- round(
  district_parcels$density_contribution,
  1
)
```
...
maplibre(
  ...
  # Adding station areas as a transparent layer
  add_fill_layer(
    id = "station_areas",
    source = district_data$transit_stations,
    fill_color = "orange",
    fill_opacity = 0.2
  )
```
...
maplibre(
  ...
  add_legend(
    ...
    values = c(
      paste0(round(min(district_parcels$density_contribution)), " u/ac"),
      "0 (at target)",
      paste0("+", round(max(district_parcels$density_contribution)), " u/ac")
    ),
...
| **Capacity prior** | Exponential prior on total capacity above min_capacity: π(cap) ∝ exp(−λ × (cap − min_capacity)). Higher λ penalizes excess capacity more steeply (half-life = ln 2 / λ) |
| **K prior** | Geometric prior on number of secondary blocks: π(k) ∝ exp(−λ × k). Higher λ favors fewer secondaries (each additional block multiplies density by exp(−λ)) |

### District Constraints

```{r}
#| label: district-constraints

# Print district constraints in a table with labels, value, description and District name in caption

caption <- paste0(
  "District Constraints for ",
  district_paths$district_name,
  " - A ",
  district_paths$district_type,
  " district"
)


scalar_constraints <- list(
  min_capacity         = constraints$min_capacity,
  min_area             = constraints$min_area,
  min_density          = constraints$min_density,
  min_lcc_fraction     = constraints$min_lcc_fraction,
  station_capacity_pct = constraints$station_capacity_pct,
  station_area_pct     = constraints$station_area_pct
)

knitr::kable(
  data.table::data.table(
    Constraint = c("Minimum capacity", "Minimum area", "Minimum density", "LCC fraction", "Station Capacity %", "Station Area %"),
    Value = unlist(scalar_constraints),
    Description = c(
      "Total housing units the district must support",
      "Total land area in the overlay district",
      "Gross density across the district",
      "At least half of capacity in the largest connected component",
      "Percentage of total capacity that must be within half mile radius of a station",
      "Percentage of total area that must be within half mile radius of a station"
    )
  ),
  caption = caption,
  booktabs = TRUE,
...
The map below shows which parcels are connected to each other. A line between two parcels indicates they are considered adjacent.

```{r}
#| label: parcel-connectivity-map
#| fig-height: 6
#| fig-width: 10
#| eval: false # large map mostly only needed for debugging, can be re-enabled to render the map

# Build points from stored projected centroids, then reproject to WGS84
parcel_centroids <- district_parcels |>
  sf::st_drop_geometry() |>
  dplyr::select(LOC_ID, centroid_x, centroid_y) |>
  sf::st_as_sf(coords = c("centroid_x", "centroid_y"), crs = 26986) |>  # replace 26986 with your actual CRS
  sf::st_transform(4326) |>
  dplyr::mutate(
    centroid_lon = sf::st_coordinates(geometry)[, 1],
    centroid_lat = sf::st_coordinates(geometry)[, 2]
  ) |>
  sf::st_drop_geometry() |>
  dplyr::select(LOC_ID, centroid_lon, centroid_lat)

# Build a linestring for each edge connecting two adjacent parcel centroids
edge_list <- igraph::as_edgelist(adjacency_graph)

# Pre-index district_parcels by parcel_id for fast lookup
parcels_indexed <- district_parcels |>
  dplyr::select(LOC_ID, centroid_x, centroid_y) |>
  sf::st_drop_geometry()
...
edge_df <- data.frame(
  ...
  dplyr::left_join(parcel_centroids, by = c("id_from" = "LOC_ID")) |>
  dplyr::rename(x_from = centroid_lon, y_from = centroid_lat) |>
  dplyr::left_join(parcel_centroids, by = c("id_to" = "LOC_ID")) |>
  dplyr::rename(x_to = centroid_lon, y_to = centroid_lat) |>
  tidyr::drop_na()

edge_long <- edge_df |>
  dplyr::mutate(linestring_id = dplyr::row_number()) |>
  tidyr::pivot_longer(
    cols = c(x_from, y_from, x_to, y_to),
    names_to = c(".value", "point"),
    names_pattern = "(x|y)_(from|to)"
  ) |>
  dplyr::arrange(linestring_id, point)

adjacency_sf <- sfheaders::sf_linestring(
  obj          = edge_long,
  x            = "x",
  y            = "y",
  ...
  sf::st_set_crs(sf::st_crs(district_parcels))
...
If there are disconnected parcels, they are shown in red on the map below.
```{r}
...
if (n_disconnected > 0) {
  ...
  parcel_geom <- sf::st_simplify(district_data$district_geometry, dTolerance = 10)
...
#| fig-height: 4
#| fig-width: 8
# Combine n_secondaries trajectories (post-burn-in) from all chains
valid_chains <- Filter(
  function(x) !isTRUE(x$initialization_failed),
  parcel_chain_results
)
all_n_sec <- unlist(lapply(valid_chains, function(x) {
  strip_burnin(x$diagnostics$n_secondaries_trajectory)
}))
...
### Total Zoning Capacity Distribution

```{r}
#| label: parcel-total-capacity-distribution
#| fig-height: 4
#| fig-width: 8
# Combine total capacity trajectories (post-burn-in) from all chains
valid_chains_cap <- Filter(
  function(x) !isTRUE(x$initialization_failed),
  parcel_chain_results
)
all_total_cap_hist <- unlist(lapply(valid_chains_cap, function(x) {
  strip_burnin(x$diagnostics$capacity_trajectory)
}))

cap_df <- data.table::data.table(capacity = all_total_cap_hist)

ggplot(cap_df, aes(x = capacity)) +
  geom_histogram(
    bins = 40,
    fill = "steelblue",
    color = "white",
  ...
  geom_vline(
    xintercept = constraints$min_capacity,
    linetype = "dashed",
    color = "red",
    linewidth = 0.8
  ) +
  annotate(
    "text",
    x = constraints$min_capacity,
    y = Inf,
    label = paste("min_capacity =", constraints$min_capacity),
    hjust = -0.1,
    vjust = 1.5,
    color = "red",
    size = 3.5
  ...
  labs(
    title = "Total Zoning Capacity Distribution (All Chains)",
    x = "Total Capacity (housing units)",
    y = "Frequency"
  ) +
  theme_minimal()
```

**Range:** `r scales::comma(min(all_total_cap_hist))` – `r scales::comma(max(all_total_cap_hist))` units

**Mean:** `r scales::comma(round(mean(all_total_cap_hist)))` units

**Median:** `r scales::comma(round(median(all_total_cap_hist)))` units
...
base_geom <- district_data$district_geometry |>
...
# Dynamic chain colors
chain_colors <- c("#1b9e77", "#d95f02", "#7570b3", "#e7298a", "#d95f02", "#8c510a")[seq_along(valid_chains)]
...
parcel_bg <- sf::st_simplify(district_data$district_geometry, dTolerance = 10)
