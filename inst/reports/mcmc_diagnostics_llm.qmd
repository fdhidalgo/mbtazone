...
execute:
  ...
  message: false
params:
  temp_store_path: "/ext/_targets_Norwood"
---
...
DISCOVERY_CAPACITY_MULTIPLIER <- config_env$DISCOVERY_CAPACITY_MULTIPLIER
MCMC_STEPS_MACRO <- config_env$MCMC_STEPS_MACRO
MCMC_BURN_IN <- config_env$MCMC_BURN_IN
...
strip_burnin <- function(traj) {
  if (MCMC_BURN_IN > 0 && length(traj) > MCMC_BURN_IN) {
    traj[(MCMC_BURN_IN + 1):length(traj)]
  } else {
    traj
  }
}
...
diag_vec <- function(chains, field) {
  unlist(lapply(chains, function(ch) {
    ...
    if (is.null(value)) numeric(0) else strip_burnin(value)
  }), use.names = FALSE)
}
...
- Discovery capacity multiplier: `r DISCOVERY_CAPACITY_MULTIPLIER`×
- MCMC steps per chain: `r MCMC_STEPS_MACRO`
- Burn-in: `r MCMC_BURN_IN`
