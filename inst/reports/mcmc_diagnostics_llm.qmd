...
JOINT_CORE_P_KEEP <- config_env$JOINT_CORE_P_KEEP
JOINT_LCC_EXCESS_RATE <- config_env$JOINT_LCC_EXCESS_RATE
JOINT_ADD_WEIGHT_NOTHING <- config_env$JOINT_ADD_WEIGHT_NOTHING
...
- Joint core p_keep: `r JOINT_CORE_P_KEEP`
- Joint LCC excess rate: `r JOINT_LCC_EXCESS_RATE`
- Joint add weight (nothing): `r JOINT_ADD_WEIGHT_NOTHING`
...
```{r}
#| label: joint-refresh-reasons
# Aggregate joint-refresh rejection reasons across chains
all_replace_reasons <- diag_dt_fallback(
  valid_chains,
  "joint_refresh_reasons",
  "replace_lcc_reasons"
)
...
if (length(all_replace_reasons) > 0) {
  ...
  cat(
    "No joint core-refresh rejection data available (replace_lcc kernel may have 0% probability).\n"
  )
}
```

```{r}
#| label: joint-refresh-constraints
# Aggregate joint-refresh constraint failures
all_replace_constraints <- diag_dt_fallback(
  valid_chains,
...
if (length(all_replace_constraints) > 0) {
  ...
  cat(
    "No joint core-refresh constraint tracking data (replace_lcc kernel may have 0% probability).\n"
  )
}
```
...
```{r}
#| label: joint-refresh-mh
# MH acceptance probability distribution
all_accept_probs <- diag_vec_fallback(
  valid_chains,
  "joint_refresh_accept_prob",
  "replace_lcc_accept_prob"
)
all_log_q_forward <- diag_vec(valid_chains, "joint_refresh_log_q_forward")
all_log_q_reverse <- diag_vec(valid_chains, "joint_refresh_log_q_reverse")
all_log_q_ratios <- diag_vec_fallback(
  valid_chains,
  "joint_refresh_log_q_ratio",
  "replace_lcc_log_q_ratio"
)
all_core_size <- diag_vec_fallback(
  valid_chains,
  "joint_refresh_core_size",
  "replace_lcc_k_retained"
)
all_n_added <- diag_vec(valid_chains, "joint_refresh_n_added")
all_n_removed <- diag_vec(valid_chains, "joint_refresh_n_removed")
all_candidate_lccs <- diag_vec_fallback(
  valid_chains,
  "joint_refresh_candidate_lccs",
  "replace_lcc_n_similar_forward"
)
all_changed_lcc <- diag_vec(valid_chains, "joint_refresh_changed_lcc")
...
if (length(all_log_q_forward) > 0 && length(all_log_q_reverse) > 0) {
  log_q_dt <- data.table(
    Quantity = c("Forward log q", "Reverse log q", "Reverse - Forward"),
    Mean = c(
      round(mean(all_log_q_forward), 2),
      round(mean(all_log_q_reverse), 2),
      round(mean(all_log_q_reverse - all_log_q_forward), 2)
    ),
    Median = c(
      round(median(all_log_q_forward), 2),
      round(median(all_log_q_reverse), 2),
      round(median(all_log_q_reverse - all_log_q_forward), 2)
    ),
    Min = c(
      round(min(all_log_q_forward), 2),
      round(min(all_log_q_reverse), 2),
      round(min(all_log_q_reverse - all_log_q_forward), 2)
    ),
    Max = c(
      round(max(all_log_q_forward), 2),
      round(max(all_log_q_reverse), 2),
      round(max(all_log_q_reverse - all_log_q_forward), 2)
    )
  )
  kable(log_q_dt)
} else if (length(all_log_q_ratios) > 0) {
...
shape_metric_dt <- rbindlist(Filter(Negate(is.null), list(
  if (length(all_core_size) > 0) {
    data.table(
      Metric = "Retained core size",
      Mean = round(mean(all_core_size), 2),
      Median = round(median(all_core_size), 2),
      P90 = round(as.numeric(stats::quantile(all_core_size, 0.9)), 2),
      Max = round(max(all_core_size), 2)
    )
  },
  if (length(all_n_removed) > 0) {
    data.table(
      Metric = "Removed secondaries",
      Mean = round(mean(all_n_removed), 2),
      Median = round(median(all_n_removed), 2),
      P90 = round(as.numeric(stats::quantile(all_n_removed, 0.9)), 2),
      Max = round(max(all_n_removed), 2)
    )
  },
  if (length(all_n_added) > 0) {
    data.table(
      Metric = "Added secondaries",
      Mean = round(mean(all_n_added), 2),
      Median = round(median(all_n_added), 2),
      P90 = round(as.numeric(stats::quantile(all_n_added, 0.9)), 2),
      Max = round(max(all_n_added), 2)
    )
  },
  if (length(all_candidate_lccs) > 0) {
    data.table(
      Metric = "Candidate LCC count",
...
if (length(all_changed_lcc) > 0) {
  cat(sprintf(
    "\n- Proposals changing the LCC: %.1f%%\n",
    100 * mean(as.logical(all_changed_lcc))
  ))
}
```
...
```{r}
#| label: joint-refresh-geometry
all_similar_fwd <- diag_vec_fallback(
  valid_chains,
  "joint_refresh_candidate_lccs",
  "replace_lcc_n_similar_forward"
)
all_similar_rev <- diag_vec_fallback(
  valid_chains,
...
health_dt <- data.table(
  Check = c(
    ...
    "LCC Exploration",
    bd_label,
    "Joint Core Refresh Feasibility",
