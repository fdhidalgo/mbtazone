...
# Find targets store
store_path <- if (dir.exists(params$temp_store_path)) params$temp_store_path else paste0("../../", params$temp_store_path)
tar_config_set(store = store_path)

# Source functions through package
library(mbtazone)

# Load the live MCMC tuning constants used by the targets pipeline.
find_repo_file <- function(rel_path) {
  candidates <- c(
    rel_path,
    file.path("..", rel_path),
    file.path("../..", rel_path),
  ...
  if (length(existing) == 0) {
    stop(sprintf("Could not locate repository file: %s", rel_path))
  }
  existing[[1]]
}

config_env <- new.env(parent = globalenv())
source(find_repo_file("inst/targets/temp_targets_config.R"), local = config_env)
source(find_repo_file("inst/targets/temp_targets_parcel_config.R"), local = config_env)

CAPACITY_PRIOR_LAMBDA <- config_env$CAPACITY_PRIOR_LAMBDA
K_PRIOR_LAMBDA <- config_env$K_PRIOR_LAMBDA
SECONDARY_AREA_THRESHOLD <- config_env$SECONDARY_AREA_THRESHOLD
REPLACE_LCC_CAP_TOLERANCE <- config_env$REPLACE_LCC_CAP_TOLERANCE
SWAP_CAP_TOLERANCE <- config_env$SWAP_CAP_TOLERANCE
...
if (length(valid_chains) == 0) {
  stop("No valid parcel MCMC chains available for diagnostics report.")
}
first_valid_chain <- valid_chains[[1]]

diag_vec <- function(chains, field) {
  unlist(lapply(chains, function(ch) {
    value <- ch$diagnostics[[field]]
    if (is.null(value)) numeric(0) else value
  }), use.names = FALSE)
}

diag_vec_fallback <- function(chains, primary, fallback = NULL) {
  values <- diag_vec(chains, primary)
  if (length(values) > 0 || is.null(fallback)) {
    return(values)
  }
  diag_vec(chains, fallback)
}

diag_dt_fallback <- function(chains, primary, fallback = NULL) {
  dt_list <- lapply(chains, function(ch) {
    dt <- ch$diagnostics[[primary]]
    if (is.null(dt) && !is.null(fallback)) {
      dt <- ch$diagnostics[[fallback]]
    }
    dt
  })
  Filter(Negate(is.null), dt_list)
}

all_lcc_cap <- diag_vec(valid_chains, "lcc_capacity_trajectory")
all_n_sec <- diag_vec(valid_chains, "n_secondaries_trajectory")
all_total_cap <- diag_vec(valid_chains, "capacity_trajectory")
all_n_comp <- diag_vec(valid_chains, "n_components_trajectory")
min_cap <- constraints$min_capacity
all_sec_cap <- all_total_cap - all_lcc_cap
all_excess_cap <- all_total_cap - min_cap

purge_results <- rbindlist(lapply(seq_along(valid_chains), function(i) {
  pr <- valid_chains[[i]]$diagnostics$purge_results
  if (is.null(pr) || length(pr) == 0) return(NULL)
  dt <- rbindlist(pr, fill = TRUE)
  if (nrow(dt) == 0) return(NULL)
  dt[, chain := names(valid_chains)[i]]
  dt
}), fill = TRUE)

birth_death_attempts <- rbindlist(lapply(seq_along(valid_chains), function(i) {
  attempts <- valid_chains[[i]]$diagnostics$birth_death_attempts
  if (is.null(attempts) || nrow(attempts) == 0) return(NULL)
  dt <- copy(attempts)
  dt[, chain := names(valid_chains)[i]]
  dt
}), fill = TRUE)
...
- Capacity prior λ: `r CAPACITY_PRIOR_LAMBDA`
- Secondary-count prior λ: `r K_PRIOR_LAMBDA`
- Secondary minimum area: `r SECONDARY_AREA_THRESHOLD` acres
- Replace-LCC capacity tolerance: `r REPLACE_LCC_CAP_TOLERANCE`
- Swap capacity tolerance: `r SWAP_CAP_TOLERANCE`
...
sampled_mean_cap <- mean(all_lcc_cap)
sampled_median_cap <- median(all_lcc_cap)
...
sampled_high_cap_pct <- 100 * mean(all_lcc_cap >= high_cap_threshold)
...
```{r}
#| label: excess-capacity-summary
capacity_slack_dt <- data.table(
  Metric = c(
    "Minimum required capacity",
    "Mean total capacity",
    "Median total capacity",
    "Mean excess capacity",
    "Median excess capacity",
    "90th percentile excess",
    "Max excess capacity",
    "States within +100 units",
    "States within +250 units",
    "States >= 1.25× minimum",
    "States >= 1.5× minimum"
  ),
  Value = c(
    round(min_cap, 0),
    round(mean(all_total_cap), 0),
    round(median(all_total_cap), 0),
    round(mean(all_excess_cap), 0),
    round(median(all_excess_cap), 0),
    round(as.numeric(quantile(all_excess_cap, 0.9, na.rm = TRUE)), 0),
    round(max(all_excess_cap), 0),
    sprintf("%.1f%%", 100 * mean(all_excess_cap <= 100)),
    sprintf("%.1f%%", 100 * mean(all_excess_cap <= 250)),
    sprintf("%.1f%%", 100 * mean(all_total_cap >= 1.25 * min_cap)),
    sprintf("%.1f%%", 100 * mean(all_total_cap >= 1.5 * min_cap))
  )
)
kable(capacity_slack_dt)
```

### Excess Capacity by Secondary Count (k)

*Links secondary count to how far sampled plans sit above the minimum requirement.*

```{r}
#| label: excess-capacity-by-k
capacity_by_k <- data.table(
  k = all_k,
  total_cap = all_total_cap,
  excess_cap = all_excess_cap
)[,
  .(
    n_samples = .N,
    mean_total = round(mean(total_cap), 0),
    mean_excess = round(mean(excess_cap), 0),
    median_excess = round(median(excess_cap), 0),
    `Pct <= +100` = sprintf("%.1f%%", 100 * mean(excess_cap <= 100))
  ),
  by = k
][order(k)]
...
kable(
  capacity_by_k,
  col.names = c("k", "N Samples", "Mean Total Cap", "Mean Excess", "Median Excess", "Pct <= +100")
)
```

### Purge / Standalone-LCC Diagnostic

*Assesses whether low-secondary states are absent because the LCC rarely meets the minimum requirement on its own.*

```{r}
#| label: purge-or-standalone
if (nrow(purge_results) > 0) {
  purge_summary <- purge_results[, .(
    n = .N,
    pct = sprintf("%.1f%%", 100 * .N / nrow(purge_results))
  ), by = .(feasible, constraint_failed)][order(-n)]

  cat("**Full purge diagnostic available:**\n\n")
  kable(
    purge_summary,
    col.names = c("Feasible After Dropping Secondaries", "Constraint Failed", "Count", "Pct")
  )
} else {
  standalone_proxy <- data.table(
    lcc_meets_min = all_lcc_cap >= min_cap,
    k = all_k,
    total_cap = all_total_cap,
    excess_cap = all_excess_cap
  )[, .(
  ...
  standalone_proxy[, pct := sprintf("%.2f%%", 100 * n_steps / sum(n_steps))]
  standalone_proxy[, lcc_meets_min := ifelse(lcc_meets_min, "Yes", "No")]
  setorder(standalone_proxy, -lcc_meets_min)

  cat("Full purge diagnostic results are not present in these chain outputs.\n\n")
  cat("Capacity-only proxy below: `LCC >= min_capacity` means the main component alone clears the legal capacity threshold.\n\n")
  kable(
    standalone_proxy[, .(
      lcc_meets_min,
      n_steps,
    ...
    col.names = c("LCC >= min_capacity", "N Steps", "Pct", "Mean k", "Mean Total Cap", "Mean Excess")
  )
}
```
...
- Mean: `r round(mean(all_lcc_cap), 0)`
- SD: `r round(sd(all_lcc_cap), 0)`
- Range: `r min(all_lcc_cap)` - `r max(all_lcc_cap)`

```{r}
#| label: k0-chain-diagnostic-llm
#| results: asis
# Detect k=0 chains by name prefix
k0_names <- grep("^k0_", names(parcel_chain_results), value = TRUE)
kpos_names <- setdiff(names(parcel_chain_results), k0_names)

if (length(k0_names) > 0 && length(kpos_names) > 0) {
  cat("\n### k=0 Chain Initialization Diagnostic\n\n")

  # Helper: extract post-burn-in secondary count trajectory
  get_post_burnin_k <- function(ch) {
    n_sec <- ch$diagnostics$n_secondaries_trajectory
    burnin <- max(1, length(n_sec) %/% 2)
    n_sec[burnin:length(n_sec)]
  }

  get_post_burnin_cap <- function(ch) {
    cap <- ch$diagnostics$capacity_trajectory
    burnin <- max(1, length(cap) %/% 2)
    cap[burnin:length(cap)]
  }

  get_post_burnin_lcc_cap <- function(ch) {
    lcc_cap <- ch$diagnostics$lcc_capacity_trajectory
    burnin <- max(1, length(lcc_cap) %/% 2)
    lcc_cap[burnin:length(lcc_cap)]
  }

  # k>0 chain stats
  kpos_k     <- unlist(lapply(parcel_chain_results[kpos_names], get_post_burnin_k))
  kpos_cap   <- unlist(lapply(parcel_chain_results[kpos_names], get_post_burnin_cap))
  kpos_lcc   <- unlist(lapply(parcel_chain_results[kpos_names], get_post_burnin_lcc_cap))
  kpos_k0pct <- mean(kpos_k == 0)

  # k=0 chain stats
  k0_k       <- unlist(lapply(parcel_chain_results[k0_names], get_post_burnin_k))
  k0_cap     <- unlist(lapply(parcel_chain_results[k0_names], get_post_burnin_cap))
  k0_lcc     <- unlist(lapply(parcel_chain_results[k0_names], get_post_burnin_lcc_cap))
  k0_k0pct   <- mean(k0_k == 0)

  comparison_dt <- data.table(
    Metric = c("Time at k=0", "Mean k", "Mean capacity", "Mean LCC capacity"),
    `k>0 Chains (mean)` = c(
      sprintf("%.1f%%", 100 * kpos_k0pct),
      round(mean(kpos_k), 2),
      round(mean(kpos_cap), 0),
      round(mean(kpos_lcc), 0)
    ),
    `k=0 Chains (mean)` = c(
      sprintf("%.1f%%", 100 * k0_k0pct),
      round(mean(k0_k), 2),
      round(mean(k0_cap), 0),
      round(mean(k0_lcc), 0)
    )
  ...
  print(kable(comparison_dt))
  cat("\n\n")

  # Interpret drift behavior
  k0_mean_k   <- mean(k0_k)
  kpos_mean_k <- mean(kpos_k)

  if (k0_mean_k < 3) {
    cat("**k=0 chains stayed at low k** (mean k =", round(k0_mean_k, 2), "after burn-in).\n\n")
    cat("This suggests the posterior is multimodal in k: the k=0 / low-k region is a valid ",
        "posterior mode that k>0 chains cannot reach due to birth/death pool asymmetry. ",
        "The adopted plan (single contiguous district with 0 secondaries) is supported by the posterior.\n\n")
  } else {
    cat("**k=0 chains drifted up** to mean k =", round(k0_mean_k, 2),
        " (comparable to k>0 chains at mean k =", round(kpos_mean_k, 2), ").\n\n")
    cat("This indicates that k=0 has low posterior mass and the high-k region dominates. ",
        "The chains initialized at k=0 quickly moved to higher k values, suggesting the ",
        "no-secondary configuration is not well-supported by the posterior.\n\n")
  }
}
```
...
# Attempt-level birth/death diagnostics (preferred when available)
bd_attempts <- if (nrow(birth_death_attempts) > 0) {
  birth_death_attempts[kernel == bd_stats$move_type]
} else {
  data.table()
}

# Universe sizes fallback for older runs
bd_universe <- if (nrow(bd_attempts) > 0 && "n_universe" %in% names(bd_attempts)) {
  bd_attempts[is.finite(n_universe), n_universe]
} else if (!use_lifted) {
  unlist(lapply(valid_chains, function(ch) {
    value <- ch$diagnostics$symmetric_bd_universe_sizes
    if (is.null(value)) numeric(0) else value
  }), use.names = FALSE)
} else {
  numeric(0)
...
if (length(bd_universe) > 0) {
  ...
  cat("Universe sizes were not stored in these chain outputs.")
}
```

### Birth/Death Candidate Pools by k

*Forward proposal pool sizes for the active birth/death kernel. These are the diagnostics that matter for add/remove imbalance; `|U|` alone can hide a large `n_add / n_rem` skew.*

```{r}
#| label: birth-death-pools-by-k
if (nrow(bd_attempts) > 0) {
  pool_by_k <- bd_attempts[,
    .(
      n_attempts = .N,
      mean_add = round(mean(n_add, na.rm = TRUE), 1),
      mean_rem = round(mean(n_rem, na.rm = TRUE), 1),
      mean_add_rem_ratio = round(mean(n_add / pmax(n_rem, 1), na.rm = TRUE), 2),
      `Pct no addable` = sprintf("%.1f%%", 100 * mean(n_add == 0, na.rm = TRUE)),
      `Pct no removable` = sprintf("%.1f%%", 100 * mean(n_rem == 0, na.rm = TRUE)),
      `Accept %` = sprintf("%.1f%%", 100 * mean(accepted, na.rm = TRUE))
    ),
    by = k_before
  ][order(k_before)]
  ...
  kable(
    pool_by_k,
    col.names = c("k", "Attempts", "Mean n_add", "Mean n_rem", "Mean n_add/n_rem", "Pct No Addable", "Pct No Removable", "Accept %")
  )
} else {
  cat("Per-attempt birth/death pool diagnostics are not available in these chain outputs.")
}
```
...
```{r}
if (!is.null(parcel_separation$overlap_matrix)){
...
all_replace_reasons <- diag_dt_fallback(valid_chains, "replace_lcc_reasons")
...
all_replace_constraints <- diag_dt_fallback(valid_chains, "replace_lcc_constraints")
...
if (length(all_replace_constraints) > 0) {
  ...
  if (nrow(constraint_summary) > 0) {
    ...
    cat(
      "No replace-LCC constraint failures recorded.\n"
    )
  }
} else {
  cat(
...
all_accept_probs <- diag_vec(valid_chains, "replace_lcc_accept_prob")
all_log_q_ratios <- diag_vec(valid_chains, "replace_lcc_log_q_ratio")
all_k_retained <- diag_vec(valid_chains, "replace_lcc_k_retained")
all_candidate_lccs <- diag_vec(valid_chains, "replace_lcc_n_similar_forward")
...
shape_metric_dt <- rbindlist(Filter(Negate(is.null), list(
  if (length(all_k_retained) > 0) {
    data.table(
      Metric = "Retained k (secondaries)",
      Mean = round(mean(all_k_retained), 2),
      Median = round(median(all_k_retained), 2),
      P90 = round(as.numeric(stats::quantile(all_k_retained, 0.9)), 2),
      Max = round(max(all_k_retained), 2)
    )
  },
  if (length(all_candidate_lccs) > 0) {
    data.table(
      Metric = "Forward candidate LCC count",
      Mean = round(mean(all_candidate_lccs), 2),
      Median = round(median(all_candidate_lccs), 2),
      P90 = round(as.numeric(stats::quantile(all_candidate_lccs, 0.9)), 2),
      Max = round(max(all_candidate_lccs), 2)
    )
  }
)), fill = TRUE)

if (nrow(shape_metric_dt) > 0) {
  cat("\n**Replace-LCC proposal diagnostics:**\n\n")
  kable(shape_metric_dt)
}
```
...
```{r}
#| label: replace-lcc-geometry
all_similar_fwd <- diag_vec(valid_chains, "replace_lcc_n_similar_forward")
all_similar_rev <- diag_vec(valid_chains, "replace_lcc_n_similar_reverse")

if (length(all_similar_fwd) > 0 && length(all_similar_rev) > 0) {
  cat(sprintf(
    "- Forward candidate LCCs: mean=%.1f, sd=%.1f\n",
    mean(all_similar_fwd),
    sd(all_similar_fwd)
  ))
  cat(sprintf(
    "- Reverse candidate LCCs: mean=%.1f, sd=%.1f\n",
...
stored_penalties <- unlist(lapply(valid_chains, function(chain) {
  pt <- chain$diagnostics$penalty_trajectory
  if (is.null(pt)) numeric(0) else pt
}), use.names = FALSE)

if (length(stored_penalties) > 0) {
  all_penalties <- stored_penalties
  penalty_source <- "runner diagnostics"
} else {
  all_penalties <- pmax(all_excess_cap, 0) * CAPACITY_PRIOR_LAMBDA
  penalty_source <- "recomputed from capacity trajectory"
}
...
- Penalty source: `r penalty_source`
...
# 6. Joint core-refresh effectiveness
