...
### Compliance Engine (R/ directory — original modules)

6 files, ~34 exported functions implementing the Excel compliance model:

1. **`data_loaders.R`** (3 exports) — `load_municipality()`, `load_station_areas()`, `load_calculation_layers()`
2. **`zoning_parameters.R`** (2 exports) — `extract_zoning_parameters()`, `create_zoning_parameters()`
3. **`gis_operations.R`** (7 exports) — District area, density denominator, contiguity, station overlap, exclusions, centroids, pre-computation
4. **`unit_capacity_calculations.R`** (18 exports) — One function per Excel column (N through AF). See file for full inventory.
...
| `mcmc_parcel_config.R` | `define_parcel_kernel_configs()` — MCMC kernel configuration |
...
library(targets)
tar_load(parcel_metrics, store = "ext/_targets_Norwood")

# Clean a district's targets
tar_destroy(store = "ext/_targets_Norwood")
```

Pipeline tiers: Data Prep → Parcel Graph → LCC Discovery (parallel tree + BFS) → Secondary Discovery → MCMC (4 chains via `crew`) → Quarto Diagnostics Report

### Rendering Diagnostics Reports

Reports are Quarto `.qmd` files in `inst/reports/` and render automatically as the final pipeline tier. Output goes to `ext/reports/` and `ext/llm_reports/`.
...
│   │   ├── parcels/             # Municipality shapefiles (ZIP)
│   │   ├── community_info.csv   # Community metadata & requirements
...
│       └── mcmc_diagnostics_llm.qmd
│
├── ext/                         # Runtime output (gitignored, not in package)
│   ├── _targets_{District}/     # Per-district target stores
│   ├── reports/                 # Rendered HTML reports
│   └── llm_reports/             # LLM-friendly markdown reports
│
...
### Precomputed Simulation Workflow

For batch evaluation of thousands of zoning parameter combinations on the same municipality, use `precompute_spatial_attributes()` to perform expensive spatial intersections once, then pass `precomputed = TRUE` to `evaluate_compliance()`. This gives ~1000x speedup (minutes vs. days for 10K iterations).

Use when parcels and spatial layers are fixed across iterations. Don't use for single evaluations or when district boundaries change.
...
- **`test-unit-capacity.R`** (~740 lines): Unit tests for all 18 calculation functions
- **`test-excel-regression.R`** (~620 lines): Validation against Excel model outputs
...
**Pipeline Runtime Dependencies** (not in DESCRIPTION — used by `inst/targets/` pipeline):

- `targets` + `tarchetypes`: Pipeline orchestration
- `crew`: Parallel worker management for multi-chain MCMC
- `igraph`: Graph operations in MCMC parcel modules
- `coda`: MCMC diagnostics (ESS, Gelman-Rubin)
...
6. Update this AGENTS.md if it represents a new calculation step
